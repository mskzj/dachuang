...
def changepassword(request):
    if request.session.get('username', default=None):
        if request.method == "POST":
            ...
            changepasswordform = bathhash(changepasswordform)
            ...
            if changepasswordform.is_valid():
                if password == changepasswordform.cleaned_data['originalpassword']:
                    if changepasswordform.cleaned_data['newpassword'] == changepasswordform.cleaned_data[
                        ...
                        user.password=bathhash(newpassword)
                        user.save()
...
def showuserinfo(request):
    if request.session.get('username',default=None):
        ...
        if userinfo:
            context['history_records']=History_record.objects.filter(user=user)[:20]
            context['userinfo']=userinfo[0]
            context['integral']=user.integral
            context['my_question']=Question.objects.filter(user=user)
            context['my_answer']=Comment.objects.filter(comment_user=user,comment_type=1)
            context['my_comment'] = Comment.objects.filter(comment_user=user, comment_type=0)
        else:
            context['error_msg']='该用户尚未填写个人信息'
        return render(request,'show_user_info2.html',context)
    else:
        return redirect(reverse('login'))
def bathhash(list):
    fold_num=0
